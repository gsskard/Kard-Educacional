...
// Dois modos:
//   • Disparar   — fluxo REAL do n8n (upload → cruzamento → salvar lista →
//                  disparar → log), com abas Educacional/Cobrança.
...
export default function Disparos() {
  ...
  return (
    <div className="disparos">
      ...
      <div className={modo === 'disparar' ? '' : 'oculto'}><DispararView /></div>
...
/* ==================================================================
 * MODO DISPARAR — fluxo real do n8n (abas Educacional/Cobrança)
 * ================================================================== */
...
function DispararView() {
  const [etapaKey, setEtapaKey] = useState('educacional')
  const etapa = ETAPAS[etapaKey]
  const edu = etapaKey === 'educacional'
  ...
  const conferencia = useMemo(() => {
    ...
    return {
      faltando: esperadas.filter((c) => !presentes.includes(c)),
      total: carga.registros.length,
    }
  }, [carga, etapa])
  ...
  function trocarEtapa(k) {
    setEtapaKey(k)
    setCarga(null); setNomeArquivo(''); setResultado(null); setMsg(''); setNomeLista(''); setEtiquetas('')
  }
    ...
    <>
      <div className="bloco">
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', marginBottom: 18 }}>
          <div className="abas">
            {ETAPAS_ORDEM.map((k) => (
              <button key={k} data-etapa={k} aria-pressed={etapaKey === k} onClick={() => trocarEtapa(k)}>
                {ETAPAS[k].titulo}
              </button>
            ))}
          </div>
          <span className={'selo ' + (edu ? 'edu' : 'cob')}>formato: {etapa.formato}</span>
        </div>
        ...
        >
          ...
          <div className="frase">
            {nomeArquivo
              ? nomeArquivo
              : <>Arraste o CSV ou <a onClick={() => inputArq.current?.click()}>escolha um arquivo</a></>}
          </div>
          <div className="obs">{nomeArquivo ? 'Clique abaixo para trocar' : <>Apenas .csv no formato {etapa.formato}</>}</div>
          ...
          <div style={{ marginTop: 18 }}>
            {temChaves ? (
              <div className="retorno-carga">
                <div><div className="retorno-num">{conferencia.total}</div><div className="retorno-rot">linhas lidas</div></div>
                {conferencia.faltando.length > 0 && (
                  <div><div className="retorno-num fraco">{conferencia.faltando.length}</div><div className="retorno-rot">colunas faltando</div></div>
                )}
                <div style={{ fontSize: 13, color: 'var(--fraco)', marginLeft: 'auto' }}>{nomeArquivo}</div>
              </div>
            ) : (
              <div className="erro-box">
                O arquivo precisa das colunas <b>cnpj</b> (ou cnpj_empregador) e <b>email</b>.
                Encontramos: {carga.cabecalho.join(', ') || 'nenhuma'}.
              </div>
            )}
            {temChaves && (
              <table>
                <thead><tr>{carga.cabecalho.slice(0, 4).map((c) => <th key={c}>{c}</th>)}</tr></thead>
                <tbody>
                  {carga.registros.slice(0, 5).map((r, i) => (
                    <tr key={i}>{carga.cabecalho.slice(0, 4).map((c) => <td key={c} className="mono">{r[c]}</td>)}</tr>
                  ))}
                </tbody>
              </table>
            )}
      ...
      {carga && temChaves && (
        <div className="bloco">
          <h2>2. Salvar como lista <small>(RF-27/28)</small></h2>
          <p className="explica">Dê um nome e etiquetas; o n8n cruza cada linha (CNPJ + e-mail) com a base prospectada.</p>
          <label className="campo"><span>Nome da lista</span>
            <input type="text" value={nomeLista} placeholder="ex.: Inadimplentes agosto" onChange={(e) => setNomeLista(e.target.value)} /></label>
          <label className="campo"><span>Etiquetas (separadas por vírgula)</span>
            <input type="text" value={etiquetas} placeholder="ex.: julho, prioridade" onChange={(e) => setEtiquetas(e.target.value)} /></label>
          <div className="acoes">
            <button className={'bt' + (edu ? '' : ' amb')} disabled={!temChaves || salvando || !nomeLista.trim()} onClick={salvarLista}>
          ...
          {resultado && (
            <div style={{ marginTop: 16 }}>
              <div className="explica"><b>{resultado.casados}</b> de <b>{resultado.total}</b> casaram com a base prospectada (elegíveis para envio).</div>
              {resultado.fora && resultado.fora.length > 0 && (
                <>
                  <div className="erro-box" style={{ marginBottom: 10 }}>{resultado.fora.length} fora da base (não recebem e-mail).</div>
                  <div className="acoes">
                    <button className="bt claro" onClick={importarFora}>Importar {resultado.fora.length} contato(s) para a base</button>
                  </div>
                </>
              )}
            </div>
...
// Seção 3 (disparo por lista salva) + log de envios (real, n8n).
function DisparoPorLista({ etapa, edu, versaoListas }) {
  ...
  useEffect(() => { setModelo((etapa.modelos && etapa.modelos[0]?.id) || ''); setListaSel(''); setFiltroEtiqueta('') }, [etapa])
  ...
  return (
    <div className="bloco">
      <h2>3. Disparo por lista <small>(RF-19/24/34)</small></h2>
      {loading ? <p className="explica">Carregando…</p> : (
        <>
          {erro && <div className="banner">{erro}</div>}
          <label className="campo"><span>Filtrar por etiqueta</span>
            <input type="text" value={filtroEtiqueta} placeholder="ex.: julho" onChange={(e) => setFiltroEtiqueta(e.target.value)} /></label>
          <label className="campo"><span>Lista salva ({filtradas.length} nesta etapa)</span>
            <select value={listaSel} onChange={(e) => setListaSel(e.target.value)}>
              <option value="">— escolha uma lista —</option>
              {filtradas.map((l) => (
                <option key={l.id} value={l.id}>{l.nome}{l.etiquetas ? ' · ' + l.etiquetas : ''} ({l.casados}/{l.total})</option>
              ))}
            </select></label>
          {etapa.modelos && etapa.modelos.length > 0 && (
            <label className="campo"><span>Modelo de e-mail</span>
              <select value={modelo} onChange={(e) => setModelo(e.target.value)}>
                {etapa.modelos.map((m) => <option key={m.id} value={m.id}>{m.nome}</option>)}
              </select></label>
          )}
          {modeloAtual && <p className="ajuda">{modeloAtual.descricao}</p>}
          <div className="acoes">
            <button className={'bt' + (edu ? '' : ' amb')} disabled={disparando || !lista || !lista.casados} onClick={disparar}>
              {disparando ? 'Disparando…' : lista ? `Disparar "${lista.nome}" (${lista.casados} casados)` : 'Disparar (escolha uma lista)'}
            </button>
            <button className="bt claro" onClick={carregar}>Atualizar</button>
          </div>
          {msg && <div className="banner">{msg}</div>}

          <p className="explica" style={{ margin: '20px 0 10px' }}>{enviosDaEtapa.length} envio(s) registrados nesta etapa.</p>
          <table>
            <thead><tr><th>Data</th><th>Lista</th><th>Etiquetas</th><th>CNPJ</th><th>E-mail</th><th>Modelo</th><th>Status</th></tr></thead>
            <tbody>
              {enviosDaEtapa.slice(0, 20).map((e, i) => (
                <tr key={e.id ?? i}>
                  <td className="mono">{e.data_envio || '—'}</td>
                  <td>{e.lista || '—'}</td>
                  <td>{e.etiquetas ? <span className="tag">{e.etiquetas}</span> : '—'}</td>
                  <td className="mono">{e.cnpj || '—'}</td>
                  <td className="mono">{e.email || '—'}</td>
                  <td>{e.modelo || '—'}</td>
                  <td><span className={'st ' + (e.status === 'enviado' ? 'ok' : 'err')}>{e.status || '—'}</span></td>
                </tr>
              ))}
              {enviosDaEtapa.length === 0 && <tr><td colSpan={7} className="vazio">Nenhum envio registrado nesta etapa.</td></tr>}
            </tbody>
          </table>
        </>
      )}
